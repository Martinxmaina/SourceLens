...
export const useChatMessages = (notebookId?: string) => {
    ...
    mutationFn: async (messageData: {
      ...
      const webhookResponse = await supabase.functions.invoke('send-chat-message', {
        body: {
          ...
          user_id: user.id,
          mindmap_topic: messageData.mindmapTopic || null
        }
      });
  ...
  // Function to get messages filtered by mindmap topic
  const getMessagesByTopic = useCallback((topic: string): EnhancedChatMessage[] => {
    if (!topic) return [];
    
    // Find all messages related to this topic
    const topicMessages: EnhancedChatMessage[] = [];
    const queryPrefix = `Discuss what these sources say about: ${topic}`;
    
    for (let i = 0; i < messages.length; i++) {
      const msg = messages[i];
      
      // Check if this is a user message asking about this topic
      if (msg.message.type === 'human') {
        const content = typeof msg.message.content === 'string' 
          ? msg.message.content 
          : '';
        
        if (content.includes(queryPrefix)) {
          // Add the question
          topicMessages.push(msg);
          
          // Add the following AI response if it exists
          if (i + 1 < messages.length && messages[i + 1].message.type === 'ai') {
